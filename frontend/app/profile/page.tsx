...
export default function Profile() {
  ...
  const handleCharacterSelect = (character_id: string) => {
    const selectedCharacter = localData?.characters.find((character: any) => character.id === character_id);
  ...
  const checkBadLocalData = (data: any) => {
    return !data || data["sofijweiofjweiofjweiofjwiofj"] === "fwaefawefwefe" || data.detail === "Invalid uid" || !data.player;
  };
  ...
  return (
    <div className='w-full h-fit relative' style={{ minHeight: "100vh" }}>
      ...
      <div className='w-full z-[900] relative'>
        <ProfilePreview playerData={localData ? localData.player : null} uid={uid || "000000000"} onUidSearch={onUidSearch} isLoading={isLoading} />

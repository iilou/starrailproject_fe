  ...
  return (
    <div className='flex items-center justify-center mt-12 relative'>
      <div className='flex flex-col justify-center border-[1px] border-w2 w-[242px] h-[315px] px-4 rounded-lg font-bold text-lg text-center'>
        <div className='mx-auto w-fit pb-2 mt-3'>
          <Image
        ...
        <div className='flex w-full justify-center gap-2 text-[#6B7433] text-xl font-extrabold items-center my-2'>
          ...
          <input
            ...
            defaultValue={uid === "000000000" ? "" : uid}
            className='w-32 bg-transparent border-none text-center text-w2 outline-none'
            onKeyDown={(e) => {
              if (e.key === "Enter") {
        ...
        {playerData && (
          <>
            <div className='text-base font-bold text-[#676767] leading-[1.25rem] mt-1'>{`Achievements: ${playerData.space_info.achievement_count}`}</div>
            <div className='text-base font-bold text-[#676767] leading-[1.25rem]'>{`Friends: ${playerData.friend_count}`}</div>
            <div className='text-base font-bold text-[#676767] leading-[1.25rem]'>{`MOC Stars: ${"N/A"}`}</div>
          </>
        )}
      </div>
      <div className='h-[315px] w-[949px] ml-12 flex flex-col justify-end'>
        <div className='text-7xl font-extrabold text-[#EEAA5B] ml-7'>
          {playerData ? playerData.nickname : "Player Name"}
        </div>
        ...
        <div className='text-xl font-medium text-[#8F8F8F] w-full px-7 border-[1px] mt-4 py-2 border-w2 rounded-lg h-[132px] flex gap-2'>
          {isLoading
            ? playerData
              ? "Refreshing ..."
              : "Loading ..."
